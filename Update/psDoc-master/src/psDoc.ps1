param(
    ...
    [parameter(Mandatory=$false, Position=1)] [string] $template = "$PSScriptRoot/out-html-template.ps1",
    ...
    [parameter(Mandatory=$false, Position=3)] [string] $extraBody = "$PSScriptRoot/extraBody.html",
    [parameter(Mandatory=$false, Position=3)] [string] $extraNav = "$PSScriptRoot/extraNav.html",
    [parameter(Mandatory=$false, Position=3)] [string] $docTitle = "$PSScriptRoot/docTitle.html",
    [parameter(Mandatory=$false, Position=3)] [string] $docFooter = "$PSScriptRoot/docFooter.html"
)
...
$i = 0
$commandsHelp = (Get-Command -module $moduleName) | get-help -full | Where-Object {! $_.name.EndsWith('.ps1')} | Where-Object {$_.Name -like "*-O365*"} | Select -Unique * | Sort-Object Name
...
$extraBody = Get-Content $extraBody -Raw -Force
$extraNav = Get-Content $extraNav -Raw -Force
$docTitle = Get-Content $docTitle -Raw -Force
$docFooter = Get-Content $docFooter -Raw -Force
